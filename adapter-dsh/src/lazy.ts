...
import type { NovelEnvelope } from './protocol.js'
 ...
 */
export function createLazySidecar(client: NovelClient, options: LazySidecarOptions): NovelSidecar {
  let handshaken = false
  let pending: Promise<void> | undefined

  const ensureHandshake = (): Promise<void> => {
    if (handshaken) return Promise.resolve()
  ...
  const gated = async (operation: () => Promise<NovelEnvelope>): Promise<NovelEnvelope> => {
    try {
  ...
  return {
    ...
    startAutorun: (projectId, fromChapter, toChapter, policy, signal) =>
      gated(() => client.startAutorun(projectId, fromChapter, toChapter, policy, signal)),
    ...
    runChapter: (projectId, chapterNumber, input, signal) =>
      gated(() => client.runChapter(projectId, chapterNumber, input, signal)),
...
function unavailableError(cause: unknown, endpoint: string): NovelAdapterError {
  return new NovelAdapterError(
    'ADAPTER_SIDECAR_UNAVAILABLE',
    `DSH Novel Sidecar is not reachable at ${endpoint}. `
      + 'Start it with "uv run dsh-novel serve" (see README section 1), then retry this tool call.',
